<template>
	<div>
		...
		<label for="ip">Nombre max de machines (sans le routeur) :</label>&nbsp;<input type="number" v-model="nbIPstr"><br>
		<label for="sr">Nombre max de sous-réseaux :</label>&nbsp;<input type="number" v-model="nbSRstr"><br>
		<button type="button" @click="calculate()">Calculer</button><br><br>
	...
	export default {
		...
		methods: {
			...
			step2() {
				...
				this.step2o_ip = "n = ln(" + this.nbIP + " + 2) / ln(2) = ~" + this.n_IP.toFixed(4);
				this.n_IP = Math.ceil(this.n_IP);
				this.step2o_ip += " → " + this.n_IP;

				this.n_SR = this.ln(this.nbSR + 2) / this.ln(2);
				this.step2o_sr = "n = ln(" + this.nbSR + " + 2) / ln(2) = ~" + this.n_SR.toFixed(4);

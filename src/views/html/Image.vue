<template>
	<div>
		...
		<pre class="language-html"><ShowCode file="html/img.html" lang="html"/></pre>
		<p>On utilise la balise <ShowCode class="language-html" inlineCode="<img />" lang="html"/> qui est une balise auto-fermante. Une balise auto-fermante est caractérisée par le slash avant le crochet droit, ce qui la ferme directement. On ne peut donc pas mettre de contenu ou d'autres balises à l'intérieur (sauf les attributs bien sûr). Ici on a d'ailleurs deux attributs obligatoires :</p>
		<ul>
		...
		<h3>Exemple audio :</h3>
		<pre class="language-html"><ShowCode file="html/audio.html" lang="html"/></pre>

		<h3>Exemple vidéo :</h3>
		<pre class="language-html"><ShowCode file="html/video.html" lang="html"/></pre>
		<p>Afin de remédier à ces problèmes de compatibilité, la plupart des sites postent leurs vidéos sur YouTube puis les intégrent via un <router-link to="/html/iframe">iframe</router-link> que nous verrons plus tard.</p>
	</div>
</template>

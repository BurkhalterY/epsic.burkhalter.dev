<template>
  <div>
    <h1>
      Importer, ajouter, modifier et supprimer des éléments d'une zone de liste
      (niveau avancé)
    </h1>
    <p>
      Ici, nous allons permettre d'interagir avec la zone de liste crée dans le
      chapitre précèdent. J'insiste sur le terme 'avancé' car nous allons
      utiliser beaucoup de VBA. J'essaierais d'expliquer au mieux.
    </p>
    <p>Commencez par créer 4 boutons :</p>
    <ul>
      <li>
        Un pour l'importation d'une adresse email (par importation, j'entends
        ajouter un email qui existe déjà)
      </li>
    ...
    <p>
      Choisissez des icônes qui vous parlent, ne leur donné pas de fonctions
      pour l'instant.
    </p>
    ...
    <p>
      ...
      formulaire, sous Événement, cliquer sur Après insertion
    </p>
    ...
    ><ShowCode file="access/Form_AfterInsert.vba" lang="vba"/></pre>
    <p>Explication du code :</p>
    <p>
      La deuxième ligne (DoCmd.RunSQL) permet d'exécuter une requête SQL, pour
      faire simple, c'est une manière de modifier la base de données depuis le
      code. Pensez a remplacer tous les noms de champs par les vôtres, sinon
      Access ne les trouveras pas.
    </p>
    <p>
      Seulement, cette fonction affiche un avertissement, nous mettons donc la
      commande DoCmd.SetWarnings False avant afin de les désactiver et
      DoCmd.SetWarnings True après pour les réactiver.
    </p>
    <p>
      DoCmd.Close ferme simplement le formulaire donc on devrait être retourné
      sur le formulaire F_Personne.
    </p>
    <p>
      Pour finir, on fait un Requery sur le champ Emails de F_Personne pour
      l'actualiser et donc afficher la nouvelle adresse ajoutée.
    </p>
    <p>
      Il faut ensuite retourner dans F_Personne et ajouter un évènement Sur clic
      au bouton + :
    </p>
    <pre
      class="language-visual-basic"
    ><ShowCode file="access/Btn_ajouter_email_Click.vba" lang="vba"/></pre>
    ...
    <p>
      ...
      associer une adresse déjà existante a la personne.
    </p>
    <p>
    ...
    <p>
      Une fois la liste déroulante créée, ajoutez un bouton, dans Événement, Sur
      clic, cela ouvre l'éditeur VBA.
    </p>
    <p>
    ...
    <pre
      class="language-visual-basic"
    ><ShowCode file="access/Btn_selectionner_Click.vba" lang="vba"/></pre>
    <p>
    ...
    <pre
      class="language-visual-basic"
    ><ShowCode file="access/Btn_importer_email_Click.vba" lang="vba"/></pre>

    <h3>Supprimer une adresse</h3>
    <p>
      Le bouton Supprimer n'aura pas pour effet de supprimer l'adresse, mais
      juste la relation entre la personne et l'adresse.
    </p>
    <p>
      Ici, pas besoin de créer de formulaire. Allez dans les propriétés du
      bouton - et ajouté un l'événement Sur clic.
    </p>
    <pre
      class="language-visual-basic"
    ><ShowCode file="access/Btn_supprimer_email_Click.vba" lang="vba"/></pre>
    <p>
      Nous avons ici 2 condition, la première c'est qu'il y ai un élément
      sélectionné dans la liste, la deuxième c'est que l'utilisateur confirme la
      suppression.
    </p>
    ...
    <p>
      Dans les événements, supprimer le Après insertion et cliquez sur Après
      MAJ.
    </p>
    <p>
    ...
    <pre
      class="language-visual-basic"
    ><ShowCode file="access/Form_AfterUpdate.vba" lang="vba"/></pre>
    <p>Il faut aussi mettre un peu de code sur le bouton d'édition :</p>
    <pre
      class="language-visual-basic"
    ><ShowCode file="access/Btn_modifier_email_Click.vba" lang="vba"/></pre>
    <p>Je fais juste un petit test pour voir si un élément est sélectionné.</p>

    <p>
      Voilà, tous les boutons devraient maintenant marcher. Le fichier est
      disponible <a href="/medias/access/Database2.accdb">ici</a> si vous voulez

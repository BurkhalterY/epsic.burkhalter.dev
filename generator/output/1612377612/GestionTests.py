# OM DE B MARS 2019 GestionTests.py
# Tout ce qui concerne la gestion des tests des films.
...
#---------------------------------------------------------------------------------------------------
# OM 2019.03.25 Définition d'une "route" /testsAfficher , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
...
#---------------------------------------------------------------------------------------------------
@tests.route('/testsAfficher', methods=['GET', 'POST'])
def varPyRouteTests():
    ...
    if request.method == "GET":
        try: 
            ...
            strsqlTestsAfficher = "SELECT * FROM t_tests"
            # OM 2019.03.25 La chaîne de caractères MySql est envoyée au serveur MySql.
            curseur_dbc.execute(strsqlTestsAfficher)
            # OM 2019.03.25 On récupère dans "dataTests" toutes les lignes et les colonnes demandées par l'instruction MySql
            dataTests = curseur_dbc.fetchall()
    ...
    return render_template('tests/TestsAfficher.html',data = dataTests, msgHtml = message_html)


#---------------------------------------------------------------------------------------------------
# OM 2019.03.17 Définition d'une "route" /testsAdd , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
#---------------------------------------------------------------------------------------------------
@tests.route('/testsAdd',methods = ['POST', 'GET'])
def testsAddPydef():

    # OM 2019.03.17 Pour
    message_html = 'Ajouter un test !'
    ...
    if request.method == 'POST':
        try:
            ...
            # OM 2019.03.17 Récupère le contenu du champ dans le formulaire HTML "TestsAdd.html"
            # OM 2019.03.17 Commande MySql pour l'insertion de la valeur "CLAVIOTTEE" dans le champ du form HTML
            # le "%s" permet d'éviter des injections SQL "simples"
            strsqlInsertTest = "INSERT INTO t_tests () VALUES ()"

            # OM 2019.03.17 Insertion de la valeur "CLAVIOTTEE" mais validée par la REGEX
            curseur_dbc.execute(strsqlInsertTest, ())

            # OM 2019.03.17 "commit" permet de finaliser la transaction !!
            connection_dbc.commit()

            # OM 2019.03.17 Commande MySql pour afficher la liste des tests classée par ordre décroissant
            # ainsi on voit la dernière valeur insérée en premier dans la liste.
            strsqlTestsAfficher = "SELECT * FROM t_tests ORDER BY id DESC"
            # OM 2019.03.17 La chaîne de caractères MySql est envoyée au serveur MySql.
            curseur_dbc.execute(strsqlTestsAfficher)

            # OM 2019.03.17 On récupère dans "dataTests" toutes les lignes et les colonnes demandées par l'instruction MySql                
            dataTests = curseur_dbc.fetchall()

            # OM 2019.03.17 Message de confirmation pour prendre soin du moral de l'utilisateur
            message_html = 'Test ajouté.'
            # OM 2019.03.17 On va afficher la liste des tests des films
            return render_template('tests/TestsAfficher.html', data = dataTests, msgHtml = message_html)


        except (pymysql.err.OperationalError, pymysql.ProgrammingError, pymysql.InternalError, pymysql.IntegrityError, TypeError) as error:
            # OM 2019.03.28 Traiter spécifiquement l'erreur MySql 1062
            # pour obtenir cette erreur 1062 il faut que la colonne "intitule_test" dans la table "t_tests"
    ...
    return render_template('tests/TestsAdd.html', msgHtml = message_html)



#---------------------------------------------------------------------------------------------------
# OM 2019.04.02 Définition d'une "route" /testsEdit , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un test de films par la commande MySql "UPDATE"
#---------------------------------------------------------------------------------------------------
@tests.route('/testsEdit',methods = ['POST', 'GET'])
def testsEditPydef():

    # OM 2019.04.02 Les données sont affichées dans un formulaire, l'affichage de la sélection
    # d'une seule ligne choisie par le bouton "edit" dans le formulaire "TestsAfficher.html"
    if request.method == 'GET':
        try:
            ...
            # OM 2019.04.02 Récupérer la valeur de "id" du formulaire html "TestAfficher.html"
            # l'utilisateur clique sur le lien "edit" et on récupére la valeur de "id" grâce à la variable "idEditHTML"
            # <a href="{{ url_for('tests.testsEditPydef', idEditHTML=row.id) }}">Edit</a>
            ...
            # OM 2019.04.02 Commande MySql pour afficher le test sélectionné dans le tableau dans le formulaire HTML
            # le "%s" permet d'éviter des injections SQL "simples"
            strsqlId = "SELECT * FROM t_tests WHERE id = %s"
            ...
            message_html = "Editer le test d'un film !!!"
    ...
    return render_template('tests/TestsEdit.html', data = dataId, msgHtml = message_html)


#---------------------------------------------------------------------------------------------------    
# OM 2019.04.02 Définition d'une "route" /testsUpdate , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# Permettre à l'utilisateur de modifier un test, et de filtrer son entrée grâce à des expressions régulières REGEXP
#---------------------------------------------------------------------------------------------------
@tests.route('/testsUpdate',methods = ['POST', 'GET'])
def testsUpdatePydef():

    # OM 2019.04.02 Pour
    message_html = 'Modifier un test !'
    ...
    if request.method == 'POST':
        try:
            ...
            # OM 2019.04.02 Récupérer la valeur de "id" du formulaire html "TestEdit.html"
            idEdit = request.form['idEditHTML']            
            # OM 2019.04.02 Récupère le contenu du champ "intitule_test" dans le formulaire HTML "TestsEdit.html"
            # OM 2019.04.02 Commande MySql pour la MODIFICATION de la valeur "CLAVIOTTEE" dans le champ "nameEditIntituleTestHTML" du form HTML "TestsEdit.html"
            # le "%s" permet d'éviter des injections SQL "simples"
            # <td><input type = "text" name = "nameEditIntituleTestHTML" value="{{ row.intitule_test }}"/></td>
            strsqlUpdateIntituleTest = "UPDATE t_tests SET  WHERE id = %s"

            # OM 2019.04.02 Insertion de la valeur "CLAVIOTTEE" mais validée par la REGEX
            curseur_dbc.execute(strsqlUpdateIntituleTest, (, idEdit,))
            # OM 2019.04.02 "commit" permet de finaliser la transaction !!
            connection_dbc.commit()

            # OM 2019.04.02 Commande MySql pour afficher la liste des tests classée par ordre décroissant
            # ainsi on voit la dernière valeur insérée en premier dans la liste.
            strsqlTestsAfficher = "SELECT * FROM t_tests ORDER BY id DESC"
            # OM 2019.04.02 La chaîne de caractères MySql est envoyée au serveur MySql.
            curseur_dbc.execute(strsqlTestsAfficher)

            # OM 2019.04.02 On récupère dans "dataTests" toutes les lignes et les colonnes demandées par l'instruction MySql                
            dataTests = curseur_dbc.fetchall()
            ...
            message_html = 'Test modifié.'
            # OM 2019.04.02 On va afficher la liste des tests des films
            return render_template('tests/TestsAfficher.html', data = dataTests, msgHtml = message_html)
            ...
            if error.args[0] == 1062:
                ...
                message_html = "La valeur " + nameTest + " existe déjà !!!"
                flash('Doublon !!! Introduire une valeur différente')
                # OM 2019.04.02 On doit afficher à nouveau le formulaire d'introduction.
                # avec ce que l'utilisateur avait sélectionné ---> Commande MySql pour Réafficher le test sélectionné 
                # le "%s" permet d'éviter des injections SQL "simples"
                strsqlIdDbl = "SELECT * FROM t_tests WHERE id = %s"
                # OM 2019.04.02 La commande MySql est envoyée à la BD
                # la valeur du "%s" est enfin associée à "idEdit"
    ...
    return render_template('tests/TestsEdit.html', msgHtml = message_html)


#---------------------------------------------------------------------------------------------------
# OM 2019.04.04 Définition d'une "route" /testsSelectDelete , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# Afficher le "test" qui va être effacé. Le "test" à effacé est choisi par le bouton "DELETE" donc il renvoie "idDeleteHTML"
#---------------------------------------------------------------------------------------------------
@tests.route('/testsSelectDelete',methods = ['POST', 'GET'])
def testsSelectDeletePydef():
    ...
    if request.method == 'GET':
        try:
            ...
            # OM 2019.04.04 Récupérer la valeur de "idDeleteHTML" du formulaire html "TestsDelete.html"
            idSelectDelete = request.args.get('idDeleteHTML')
            # OM 2019.04.04 Afficher le "test" à effacer (choisi par l'utilisateur) bouton "DELETE" du formulaire "TestsAfficher.html"
            # <a href="{{ url_for('tests.testsDeletePydef', idDeleteHTML=row.id) }}">Delete</a>
            # le "%s" permet d'éviter des injections SQL "simples"
            strsqlSelectDeleteTest = "SELECT * FROM t_tests WHERE id = %s"
            # OM 2019.04.04 La commande MySql est envoyée à la BD
            # la valeur du "%s" est enfin associée à "idSelectDelete"
            curseur_dbc.execute(strsqlSelectDeleteTest, (idSelectDelete,))
    ...
    return render_template('tests/TestsDelete.html', data = dataIdSousCategorieSelDel, msgHtml = message_html)
...
# Permettre à l'utilisateur de modifier un test, et de filtrer son entrée grâce à des expressions régulières REGEXP
#---------------------------------------------------------------------------------------------------
@tests.route('/testsDelete',methods = ['POST', 'GET'])
def testsDeletePydef():

    # OM 2019.04.02 Pour
    message_html = 'Effacer un test !'
    ...
    if request.method == 'POST':
        try:
            ...
            # OM 2019.04.02 Récupérer la valeur de "id" du formulaire html "TestsAfficher.html"
            idDelete = request.form['idDeleteHTML']            

            # OM 2019.04.02 On doit afficher à nouveau le formulaire d'introduction.
            # avec ce que l'utilisateur avait sélectionné ---> Commande MySql pour Réafficher le test sélectionné 
            # le "%s" permet d'éviter des injections SQL "simples"
            strsqlDeleteTest = "DELETE FROM t_tests WHERE id = %s"
            # OM 2019.04.02 La commande MySql est envoyée à la BD
            # la valeur du "%s" est enfin associée à "idEdit"
            curseur_dbc.execute(strsqlDeleteTest, (idDelete,))
            # OM 2019.04.02 "commit" permet de finaliser la transaction !!
            connection_dbc.commit()
            ...
            # Cette erreur 1451, signifie qu'on veut effacer un "test" de films qui est associé dans "t_test_film".
            if error.args[0] == 1451:
                ...
                # OM 2019.04.02 On récupère dans "dataTests" toutes les lignes et les colonnes demandées par l'instruction MySql                
                dataTestsAfterDelete = curseur_dbc.fetchall()
                ...
                # OM 2019.04.02 On va afficher la liste des tests des films
                return render_template('tests/TestsAfficher.html', data = dataTestsAfterDelete, msgHtml = message_html)
            else:
                # OM 2019.04.02 Trois façons de communiquer qu'une autre erreur que la 1062 est survenue.
                print( 'MySQL?????%d?%s' % (error.args[ 0 ], error.args[ 1 ]) )
                message_html = "Problème monstrueux !!! "
    ...
    # OM 2019.04.02 Envoie la page "HTML" au serveur. On passe un message d'information dans "message_html"
    return render_template('tests/TestsAfficher.html',  msgHtml = message_html)
